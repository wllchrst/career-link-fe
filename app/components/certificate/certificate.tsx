import certificatebg from "~/asset/certificate.png"
    ...
    return (
        <div className={"object-cover w-full h-2/5 relative"} id="certificate">
            ...
            <div className="absolute w-4/5 top-1/6 left-1/2 -translate-x-1/2 text-center flex flex-col gap-5">
                <h4 className="text-3xl w-full text-[var(--accent)] font-semibold font-serif">Certificate of {certificate.type == 'premium'?"Achievement":"Participation"}</h4>
                <p className="font-serif text-sm">This certifies that</p>
                <h1 className="text-4xl w-full text-sky-500/100 dark:text-sky-400/100 font-bold font-serif">{certificate.user.name}</h1>
                <p className="font-serif text-sm">has successfully completed all sessions and project required for</p>
                <h4 className="text-4xl w-full text-[var(--accent)] font-medium font-serif">{certificate.bootcamp.name}</h4>
